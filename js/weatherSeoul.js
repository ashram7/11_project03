$(function(){
	...
	$.getJSON('http://api.openweathermap.org/data/2.5/weather?id=1835848&appid=e16562d519a8afd00cb9f02a17662dfb&units=metric',function(data){
		...
		$('.Sctemp').text(Math.floor($cTemp)+"°");
		$('.Sclowtemp').text("최저 "+Math.floor($minTemp)+"°");
		$('.Schightemp').text("최고 "+Math.floor($maxTemp)+"°");
		$('.Scfeel').text("체감 "+Math.floor($cfeel)+"°");
		$('.Schumi').text("습도 "+Math.floor($chumi)+"°");
		$('.SWsubTit').text("( "+$cDate+" )");
		$('.Scicon>img').remove();
